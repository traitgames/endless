...
6. `set_fog`: `{"type":"set_fog","density":number,"colorHex":"#RRGGBB"}`
7. `set_terrain_color`: `{"type":"set_terrain_color","colorHex":"#RRGGBB"}`
8. `set_trees`: `{"type":"set_trees","density":number,"trunkColor":"#RRGGBB","canopyColor":"#RRGGBB"}`
9. `spawn_landmark`: `{"type":"spawn_landmark","kind":"pillar|beacon","x":number,"z":number,"yOffset":number,"scale":number,"colorHex":"#RRGGBB"}`
10. `clear_landmarks`: `{"type":"clear_landmarks"}`
11. `run_local_world_command`: `{"type":"run_local_world_command","command":"string"}`
...
- Frontend-local commands and exact syntax/variants are injected into prompt context from the frontend help catalog. Prefer those forms when they match the user request.
